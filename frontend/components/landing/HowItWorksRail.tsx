...
function MockPendingRow({
  title,
  bird,
  flyingLaterLabel = false,
}: MockTask & { flyingLaterLabel?: boolean }) {
  return (
    <div className="flock-list-item rounded-lg border border-stone/20 bg-surface/40 px-3 py-2">
      <div className="flock-row">
        ...
        <div className="flex min-w-0 flex-1 flex-col xl:flex-row xl:items-center">
          ...
          <div className="flock-row__actions pb-1.5 xl:pb-0">
            <MockDragHandle />
            <MockTrashIcon />
          </div>
        </div>
    ...
    <div className="flock-list-item rounded-lg border border-stone/15 bg-surface/25 px-3 py-2">
      <div className="flock-row">
        ...
        <div className="flex min-w-0 flex-1 flex-col xl:flex-row xl:items-center">
          ...
          <time className="pb-1.5 text-xs text-ink/40 xl:shrink-0 xl:pb-0">{time}</time>
        </div>
      </div>
    </div>
  );
...
function MockFlyingLaterView() {
  const items: MockTask[] = [
    { title: "Draft Q4 hiring brief", bird: "Artboard5.svg" },
    { title: "Review analytics cleanup ticket", bird: "Artboard11.svg" },
  ];

  return (
    <div className="flock-list space-y-2" aria-hidden="true">
      <p className="mb-3 font-display text-sm text-ink">Flying later</p>
      {items.map((item) => (
        <MockPendingRow key={item.title} {...item} flyingLaterLabel />
      ))}
      <p className="pt-1 text-xs text-ink/55">hide tasks</p>
    </div>
  );
}
...
function MockHistoryView() {
  const items: (MockTask & { time: string })[] = [
    { title: "write meeting agenda", bird: "Artboard12.svg", time: "June 19 10:27am" },
    { title: "finish chapter 2", bird: "Artboard3.svg", time: "June 19 9:14am" },
  ];

  return (
    <div className="flock-list space-y-2" aria-hidden="true">
      <p className="mb-3 font-display text-sm text-ink">Today</p>
      {items.map((item) => (
...
const STEPS: Step[] = [
  ...
  {
    number: "3",
    title: "Use Flying later when priorities shift",
    body: 'And because priorities change throughout the day, you can move tasks into a "later list" for consideration after the most important tasks are done.',
    illustration: <MockFlyingLaterView />,
  },
  {
    number: "4",
    title: "Finish and move on",
    body: "Mark the bird done, and the next one appears. The history page keeps an exportable record of what you finished.",
    ...
    <div className="relative">
      ...
      <ol className="space-y-16 sm:space-y-20">
        {STEPS.map((step) => (
          <li
            key={step.number}
            className="relative z-10 grid gap-8 xl:grid-cols-[1fr_minmax(0,360px)] xl:gap-10"
          >
